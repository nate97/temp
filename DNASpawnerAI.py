...
class DNASpawnerAI:
    ...
    def _createObjects(self, group, zone):
        ...
        if group.getName()[:13] == 'fishing_pond_':
            ...
            picnicTable = DistributedPicnicBasketAI.DistributedPicnicBasketAI(simbase.air, nameInfo[2], pos[0], pos[1],
                                                                              pos[2], hpr[0], hpr[1], hpr[2])
            picnicTable.generateWithRequired(zone)
            picnicTable.start()

        elif (group.getName() == 'prop_game_table_DNARoot' or group.getName()[:13] == 'game_table_') and config.GetBool(
                'want-picnic-games', True):
            pos = group.getPos()
            hpr = group.getHpr()
            nameInfo = group.getName().split('_')
            tableIndex = int(group.getParent().getName().split('_')[-1])
            picnicTable = DistributedPicnicTableAI.DistributedPicnicTableAI(simbase.air, zone, nameInfo[2], pos[0],
            ...
            kart = DistributedGolfKartAI.DistributedGolfKartAI(simbase.air, golfCourse, pos[0], pos[1], pos[2], hpr[0],
                                                               hpr[1], hpr[2])
